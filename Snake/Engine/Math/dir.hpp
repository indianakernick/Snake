...
namespace Math {
  ...
  ///Get the first direction in the pair (most sig)
  constexpr Dir first(const DirPair pair) {
    return static_cast<Dir>(static_cast<DirType>(pair) >> DirType(2));
  }
  
  ///Get the second direction in the pair (least sig)
  constexpr Dir second(const DirPair pair) {
    return static_cast<Dir>(static_cast<DirType>(pair) & DirType(2));
  }
  
  ///Are the two directions equal?
  constexpr bool pairEqual(const DirPair pair) {
    return first(pair) == second(pair);
  }
  
  ///Get the opposite directions of a pair of directions
